    ...
    public static class MethodInfoHelper
    {
        public static MethodInfo MethodOf<T>(Expression<Action<T>> methodCall)
        {
            ...
            if (!(methodCall.Body is MethodCallExpression))
                throw new ArgumentException("Expression must be a method call", "methodCall");
        ...
        public static MethodInfo MethodOf<TTarget, TOutput>(Expression<Func<TTarget, TOutput>> methodCall)
        {
            ...
            if (!(methodCall.Body is MethodCallExpression))
                throw new ArgumentException(
                    string.Format("Expression must be a method call, was {0}", methodCall.NodeType), "methodCall");
